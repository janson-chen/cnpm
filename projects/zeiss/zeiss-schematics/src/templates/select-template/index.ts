import { Rule, SchematicContext, Tree, apply, chain, mergeWith, template, url, move, } from '@angular-devkit/schematics';
...
export default function (options: any): Rule {
    ...
    return chain([
        ...
        mergeWith(apply(url('./files'), [
            ...
            move(/(.*)(src[\\|\/]app.*)/i.test(path.resolve('.')) ? path.resolve('.').replace(/(.*)(src[\\|\/]app.*)/i, '$2') : '')
        ]))
    ]);
}
